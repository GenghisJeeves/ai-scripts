...
if [[ $(lshw -C display 2>/dev/null | grep -i vendor | grep -oi 'Nvidia') =~ [Nn][Vv][Ii][Dd][Ii][Aa] ]]; then
    ...
    wget -nv https://github.com/LostRuins/koboldcpp/releases/latest/download/koboldcpp-linux-x64 && mv koboldcpp-linux-x64 ~/koboldcpp && chmod +x ~/koboldcpp &
    ...
    wget -nv https://koboldai.org/cpplinuxrocm && mv cpplinuxrocm ~/koboldcpp && chmod +x ~/koboldcpp &
...
~/koboldcpp --model $GGUF_MODEL_PATH --contextsize $GGUF_CONTEXT --gpulayers $GGUF_GPU_LAYERS &
